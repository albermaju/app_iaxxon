...
def get_data(time_period):
    ...
    query= f'from (bucket: "Iaxxon Bodegas_Habla")\
...
query_api = client.query_api()
query_fan = f'''from(bucket: "Iaxxon Bodegas_Habla")\
    |> range(start: -24h)\
    |> filter(fn: (r) => r["_field"] == "fan")\
    |> aggregateWindow(every: 1m, fn: last, createEmpty: false)\
    |> yield(name: "last")'''

query_pump = f'''from(bucket: "Iaxxon Bodegas_Habla")\
    |> range(start: -24h)\
